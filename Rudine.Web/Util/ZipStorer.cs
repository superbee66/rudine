...
namespace Rudine.Web.Util
{
    ...
    internal class ZipStorer : IDisposable
    {
        ...
        public void AddFile(Compression _method, string _pathname, string _filenameInZip, string _comment)
        {
            ...
            FileStream stream = new FileStream(_pathname,
                FileMode.Open,
                FileAccess.Read);
            AddStream(_method,
                _filenameInZip,
                stream,
                File.GetLastWriteTime(_pathname),
                _comment);
            stream.Close();
        }
        ...
        public void Close()
        {
            if (Access != FileAccess.Read)
            {
                ...
                if (CentralDirImage != null)
                    ZipFileStream.Write(CentralDirImage,
                        0,
                ...
                if (CentralDirImage != null)
                    WriteEndRecord(centralSize + (uint) CentralDirImage.Length,
                        centralOffset);
                else
                    WriteEndRecord(centralSize,
                        centralOffset);
            }
        ...
        public List<ZipFileEntry> ReadCentralDir()
        {
            ...
            {
                uint signature = BitConverter.ToUInt32(CentralDirImage,
                    pointer);
                if (signature != 0x02014b50)
                    break;

                bool encodeUTF8 = (BitConverter.ToUInt16(CentralDirImage,
                                       pointer + 8) & 0x0800) != 0;
                ushort method = BitConverter.ToUInt16(CentralDirImage,
                    pointer + 10);
                uint modifyTime = BitConverter.ToUInt32(CentralDirImage,
                    pointer + 12);
                uint crc32 = BitConverter.ToUInt32(CentralDirImage,
                    pointer + 16);
                uint comprSize = BitConverter.ToUInt32(CentralDirImage,
                    pointer + 20);
                uint fileSize = BitConverter.ToUInt32(CentralDirImage,
                    pointer + 24);
                ushort filenameSize = BitConverter.ToUInt16(CentralDirImage,
                    pointer + 28);
                ushort extraSize = BitConverter.ToUInt16(CentralDirImage,
                    pointer + 30);
                ushort commentSize = BitConverter.ToUInt16(CentralDirImage,
                    pointer + 32);
                uint headerOffset = BitConverter.ToUInt32(CentralDirImage,
                    pointer + 42);
                ...
                zfe.FilenameInZip = encoder.GetString(CentralDirImage,
                    pointer + 46,
                    filenameSize);
        ...
        public bool ExtractFile(ZipFileEntry _zfe, string _filename)
        {
            ...
            Stream output = new FileStream(_filename,
                FileMode.Create,
                FileAccess.Write);
            bool result = ExtractFile(_zfe,
                output);
            if (result)
                output.Close();

            File.SetCreationTime(_filename,
                _zfe.ModifyTime);
            File.SetLastWriteTime(_filename,
                _zfe.ModifyTime);
        ...
        public bool ExtractFile(ZipFileEntry _zfe, Stream _stream)
        {
            ...
            ZipFileStream.Seek(_zfe.HeaderOffset,
                SeekOrigin.Begin);
            ZipFileStream.Read(signature,
                0,
                4);
            if (BitConverter.ToUInt32(signature,
                    0) != 0x04034b50)
                return false;
            ...
            if (_zfe.Method == Compression.Store)
                ...
                inStream = new DeflateStream(ZipFileStream,
            ...
            ZipFileStream.Seek(_zfe.FileOffset,
                SeekOrigin.Begin);
        ...
        public static bool RemoveEntries(ref ZipStorer _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                ZipStorer tempZip = Create(tempZipName,
                    string.Empty);

                foreach (ZipFileEntry zfe in fullList)
                    if (!_zfes.Contains(zfe))
                        if (_zip.ExtractFile(zfe,
                            tempEntryName))
                            tempZip.AddFile(zfe.Method,
                                tempEntryName,
                                zfe.FilenameInZip,
                                zfe.Comment);
                _zip.Close();
                ...
                File.Move(tempZipName,
                    _zip.FileName);

                _zip = Open(_zip.FileName,
                    _zip.Access);
            } catch
            {
                return false;
        ...
        private uint GetFileOffset(uint _headerOffset)
        {
            ...
            ZipFileStream.Seek(_headerOffset + 26,
                SeekOrigin.Begin);
            ZipFileStream.Read(buffer,
                0,
                2);
            ushort filenameSize = BitConverter.ToUInt16(buffer,
                0);
            ZipFileStream.Read(buffer,
                0,
                2);
            ushort extraSize = BitConverter.ToUInt16(buffer,
                0);
        ...
        */
        ...
        private void WriteLocalHeader(ref ZipFileEntry _zfe)
        {
            ...
            ZipFileStream.Write(new byte[] { 80, 75, 3, 4, 20, 0 },
                0,
                6); // No extra header
        ...
        */
        ...
        private void WriteEndRecord(uint _size, uint _offset)
        {
            ...
            ZipFileStream.Write(new byte[] { 80, 75, 5, 6, 0, 0, 0, 0 },
                0,
                8);
            ZipFileStream.Write(BitConverter.GetBytes((ushort) Files.Count + ExistingFiles),
                0,
                2);
            ZipFileStream.Write(BitConverter.GetBytes((ushort) Files.Count + ExistingFiles),
                0,
                2);
            ZipFileStream.Write(BitConverter.GetBytes(_size),
                0,
                4);
            ZipFileStream.Write(BitConverter.GetBytes(_offset),
                0,
                4);
            ZipFileStream.Write(BitConverter.GetBytes((ushort) encodedComment.Length),
                0,
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (sig == 0x06054b50)
                    {
                        ...
                        ZipFileStream.Seek(centralDirOffset,
                            SeekOrigin.Begin);
                        ZipFileStream.Read(CentralDirImage,
                            0,
                            centralSize);
