...
namespace Rudine.Web.Util
{
    ...
    internal static class DynamicQueryable
    {
        ...
        public static IQueryable GroupBy(this IQueryable source, string keySelector, string elementSelector, params object[] values)
        {
            ...
            LambdaExpression keyLambda = DynamicExpression.ParseLambda(source.ElementType,
                null,
                keySelector,
                values);
            LambdaExpression elementLambda = DynamicExpression.ParseLambda(source.ElementType,
                null,
                elementSelector,
                values);
        ...
        {
            return (IQueryable<T>) OrderBy((IQueryable) source,
                ordering,
                values);
        }
        ...
        public static IQueryable OrderBy(this IQueryable source, string ordering, params object[] values)
        {
            ...
            foreach (DynamicOrdering o in orderings)
            {
                queryExpr = Expression.Call(
                    ...
                    queryExpr,
                    Expression.Quote(Expression.Lambda(o.Selector,
                        parameters)));
                methodAsc = "ThenBy";
        ...
        {
            return (IQueryable<T>) Where((IQueryable) source,
                predicate,
                values);
        }
    ...
    public abstract class DynamicClass
    {
        public override string ToString()
        {
            ...
            for (int i = 0; i < props.Length; i++)
            {
                ...
                sb.Append("=");
                sb.Append(props[i].GetValue(this,
                    null));
            }
            sb.Append("}");
            return sb.ToString();
    ...
    [Browsable(false)]
    /// <summary>
    ///     Access Modifier adjusted to public specifically for the InfoPath solution as it is now referenced outside the
    ///     Dynamic.cs file
    /// </summary>
    ...
    internal class Signature : IEquatable<Signature>
    {
        ...
        public override bool Equals(object obj) { return obj is Signature ? Equals((Signature) obj) : false; }
    ...
    internal class ClassFactory
    {
        ...
        private void GenerateEquals(TypeBuilder tb, FieldInfo[] fields)
        {
            ...
            gen.Emit(OpCodes.Isinst,
                tb);
            gen.Emit(OpCodes.Stloc,
                other);
        ...
        private FieldInfo[] GenerateProperties(TypeBuilder tb, DynamicProperty[] properties)
        {
            ...
            for (int i = 0; i < properties.Length; i++)
            {
                ...
                FieldBuilder fb = tb.DefineField("_" + dp.Name,
                    dp.Type,
        ...
        public Type GetDynamicClass(IEnumerable<DynamicProperty> properties)
        {
            ...
            try
            {
                ...
                Type type;
                if (!classes.TryGetValue(signature,
                        out type))
                {
    ...
    public sealed class ParseException : Exception
    {
        ...
        public ParseException(string message, int position)
            : base(message) { this.position = position; }
    ...
    public class ExpressionParser
    {
        ...
        private void AddSymbol(string name, object value)
        {
            if (symbols.ContainsKey(name))
                throw ParseError(Res.DuplicateIdentifier,
                    name);
            symbols.Add(name,
                value);
        }
        ...
        private void CheckAndPromoteOperands(Type signatures, string opName, ref Expression left, ref Expression right, int errorPos)
        {
            ...
            if (FindMethod(signatures,
                    ...
                    args,
                    out method) != 1)
                throw IncompatibleOperandsError(opName,
                    left,
                    right,
                    errorPos);
            left = args[0];
        ...
        private static int CompareConversions(Type s, Type t1, Type t2)
        {
            ...
            if (s == t2)
                return -1;
            bool t1t2 = IsCompatibleWith(t1,
                t2);
            bool t2t1 = IsCompatibleWith(t2,
                t1);
            if (t1t2 && !t2t1)
                return 1;
            if (t2t1 && !t1t2)
        ...
        private int FindBestMethod(IEnumerable<MethodBase> methods, Expression[] args, out MethodBase method)
        {
            MethodData[] applicable = methods.
                ...
                Where(m => IsApplicable(m,
                          args)).
                ToArray();
            if (applicable.Length > 1)
                applicable = applicable.
                    Where(m => applicable.All(n => m == n || IsBetterThan(args,
                                                       m,
                                                       n))).
                    ToArray();
            if (applicable.Length == 1)
        ...
        private static Type FindGenericType(Type generic, Type type)
        {
            while (type != null && type != typeof(object))
            {
                ...
                if (generic.IsInterface)
                    ...
                    {
                        Type found = FindGenericType(generic,
                            intfType);
                        if (found != null)
                            return found;
                    }
            ...
            foreach (Type t in SelfAndBaseTypes(type))
            {
                ...
                if (members.Length != 0)
                {
                    IEnumerable<MethodBase> methods = members.
                        ...
                        Where(m => m != null);
                    int count = FindBestMethod(methods,
                        args,
                        out method);
                    if (count != 0)
                        return count;
        ...
        private int FindMethod(Type type, string methodName, bool staticAccess, Expression[] args, out MethodBase method)
        {
            ...
            foreach (Type t in SelfAndBaseTypes(type))
            {
                MemberInfo[] members = t.FindMembers(MemberTypes.Method,
                    ...
                    methodName);
                int count = FindBestMethod(members.Cast<MethodBase>(),
                    args,
                    out method);
                if (count != 0)
                    return count;
        ...
        private Expression GenerateAdd(Expression left, Expression right)
        {
            if (left.Type == typeof(string) && right.Type == typeof(string))
                return GenerateStaticMethodCall("Concat",
                    left,
                    right);
            return Expression.Add(left,
                right);
        }
        ...
        private Expression GenerateConditional(Expression test, Expression expr1, Expression expr2, int errorPos)
        {
            if (test.Type != typeof(bool))
                throw ParseError(errorPos,
                    Res.FirstExprMustBeBool);
            if (expr1.Type != expr2.Type)
            {
                Expression expr1as2 = expr2 != nullLiteral ? PromoteExpression(expr1,
                                          expr2.Type,
                                          true) : null;
                Expression expr2as1 = expr1 != nullLiteral ? PromoteExpression(expr2,
                                          expr1.Type,
                                          true) : null;
                ...
                else
                {
                    ...
                    if (expr1as2 != null && expr2as1 != null)
                        throw ParseError(errorPos,
                            Res.BothTypesConvertToOther,
                            type1,
                            type2);
                    throw ParseError(errorPos,
                        Res.NeitherTypeConvertsToOther,
                        type1,
                        type2);
                }
            }
        ...
        private Expression GenerateConversion(Expression expr, Type type, int errorPos)
        {
            ...
            if (exprType.IsValueType && type.IsValueType)
            {
                ...
                if ((IsNumericType(exprType) || IsEnumType(exprType)) &&
                    ...
                    return Expression.ConvertChecked(expr,
                        type);
            }
        ...
        private Expression GenerateGreaterThan(Expression left, Expression right)
        {
            if (left.Type == typeof(string))
                return Expression.GreaterThan(
                    GenerateStaticMethodCall("Compare",
                        left,
                        right),
        ...
        private Expression GenerateLessThan(Expression left, Expression right)
        {
            ...
            return Expression.LessThan(left,
                right);
        }
        ...
        {
            return Expression.NotEqual(left,
                right);
        }
        ...
        {
            return Expression.Subtract(left,
                right);
        }
        ...
        /// <returns></returns>
        public static Type GetNonNullableType(Type type) { return IsNullableType(type) ? type.GetGenericArguments()[0] : type; }
        ...
        {
            return left.Type.GetMethod(methodName,
        ...
        private static bool IsEnumType(Type type) { return GetNonNullableType(type).IsEnum; }
        private static bool IsNullableType(Type type) { return type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>); }
        private static bool IsNumericType(Type type) { return GetNumericTypeKind(type) != 0; }
        ...
        private static bool IsSignedIntegralType(Type type) { return GetNumericTypeKind(type) == 2; }
        private static bool IsUnsignedIntegralType(Type type) { return GetNumericTypeKind(type) == 3; }
        ...
        public Expression Parse(Type resultType)
        {
            ...
            if (resultType != null)
                if ((expr = PromoteExpression(expr,
                         resultType,
                         true)) == null)
                    throw ParseError(exprPos,
                        Res.ExpressionTypeMismatch,
                ...
                switch (op.id)
                {
                    ...
                    case TokenId.Minus:
                        CheckAndPromoteOperands(typeof(ISubtractSignatures),
                            ...
                            ref right,
                            op.pos);
                        left = GenerateSubtract(left,
                            right);
                        break;
                    case TokenId.Amphersand:
                        left = GenerateStringConcat(left,
                            right);
                        break;
                }
            }
        ...
        private Expression ParseAggregate(Expression instance, Type elementType, string methodName, int errorPos)
        {
            ...
            if (FindMethod(typeof(IEnumerableSignatures),
                    methodName,
                    false,
                    args,
                    out signature) != 1)
                throw ParseError(errorPos,
                    Res.NoApplicableAggregate,
                    methodName);
            Type[] typeArgs;
            ...
            if (args.Length == 0)
                ...
                args = new[]
                {
                    instance, Expression.Lambda(args[0],
                        innerIt)
                };
        ...
        private Expression ParseComparison()
        {
                   ...
                   token.id == TokenId.LessThan || token.id == TokenId.LessThanEqual)
            {
                ...
                if (isEquality && !left.Type.IsValueType && !right.Type.IsValueType)
                {
                    if (left.Type != right.Type)
                        if (left.Type.IsAssignableFrom(right.Type))
                            right = Expression.Convert(right,
                                left.Type);
                        else if (right.Type.IsAssignableFrom(left.Type))
                            left = Expression.Convert(left,
                                right.Type);
                        else
                            throw IncompatibleOperandsError(op.text,
                                left,
                                right,
                                op.pos);
                } else if (IsEnumType(left.Type) || IsEnumType(right.Type))
                {
                    ...
                    {
                        Expression e;
                        if ((e = PromoteExpression(right,
                                 left.Type,
                                 true)) != null)
                            right = e;
                        else if ((e = PromoteExpression(left,
                                      right.Type,
                                      true)) != null)
                            left = e;
                        else
                ...
                switch (op.id)
                {
                    ...
                    case TokenId.DoubleEqual:
                        left = GenerateEqual(left,
                            right);
                        break;
                    case TokenId.ExclamationEqual:
                    case TokenId.LessGreater:
                        left = GenerateNotEqual(left,
                            right);
                        break;
                    case TokenId.GreaterThan:
                        left = GenerateGreaterThan(left,
                            right);
                        break;
                    case TokenId.GreaterThanEqual:
                        left = GenerateGreaterThanEqual(left,
                            right);
                        break;
                    case TokenId.LessThan:
                        left = GenerateLessThan(left,
                            right);
                        break;
                    case TokenId.LessThanEqual:
                        left = GenerateLessThanEqual(left,
                            right);
                        break;
                }
            }
        ...
        private Expression ParseElementAccess(Expression expr)
        {
            ...
            if (expr.Type.IsArray)
            {
                ...
                Expression index = PromoteExpression(args[0],
                    typeof(int),
                    true);
                if (index == null)
                    throw ParseError(errorPos,
                        Res.InvalidIndex);
                return Expression.ArrayIndex(expr,
                    index);
            }
            MethodBase mb;
            switch (FindIndexer(expr.Type,
                args,
                out mb))
            {
                case 0:
        ...
        private Expression ParseIdentifier()
        {
            ValidateToken(TokenId.Identifier);
            object value;
            if (keywords.TryGetValue(token.text,
                out value))
            {
                if (value is Type)
            ...
            if (symbols.TryGetValue(token.text,
                    out value) ||
                externals != null && externals.TryGetValue(token.text,
                    out value))
            {
            ...
            if (it != null)
                return ParseMemberAccess(null,
                    it);
            throw ParseError(Res.UnknownIdentifier,
                token.text);
        }
        ...
        private Expression ParseIif()
        {
            ...
            if (args.Length != 3)
                throw ParseError(errorPos,
                    Res.IifRequiresThreeArgs);
            return GenerateConditional(args[0],
                args[1],
                args[2],
                errorPos);
        }
        ...
        private Expression ParseIntegerLiteral()
        {
            ...
            {
                ulong value;
                if (!UInt64.TryParse(text,
                        out value))
                    throw ParseError(Res.InvalidIntegerLiteral,
                        text);
                NextToken();
        ...
        private Expression ParseLambdaInvocation(LambdaExpression lambda)
        {
            ...
            Expression[] args = ParseArgumentList();
            MethodBase method;
            if (FindMethod(lambda.Type,
                    "Invoke",
                    false,
                    args,
                    out method) != 1)
                throw ParseError(errorPos,
                    Res.ArgsIncompatibleWithLambda);
        ...
        private Expression ParseLogicalAnd()
        {
            ...
            while (token.id == TokenId.DoubleAmphersand || TokenIdentifierIs("and"))
            {
                ...
                CheckAndPromoteOperands(typeof(ILogicalSignatures),
                    op.text,
                    ref left,
                    ref right,
                    op.pos);
                left = Expression.AndAlso(left,
                    right);
            }
            return left;
        }
        ...
        private Expression ParseMemberAccess(Type type, Expression instance)
        {
            ...
            if (token.id == TokenId.OpenParen)
            {
                ...
                {
                    Type enumerableType = FindGenericType(typeof(IEnumerable<>),
                        type);
                    if (enumerableType != null)
                    {
                        Type elementType = enumerableType.GetGenericArguments()[0];
                        return ParseAggregate(instance,
                            elementType,
                            id,
                            errorPos);
                    }
            ...
            MemberInfo member = FindPropertyOrField(type,
                id,
                instance == null);
            if (member == null)
                throw ParseError(errorPos,
                   ...
                   token.id == TokenId.Percent || TokenIdentifierIs("mod"))
            {
                ...
                switch (op.id)
                {
                    case TokenId.Asterisk:
                        left = Expression.Multiply(left,
                            right);
                        break;
                    case TokenId.Slash:
                        left = Expression.Divide(left,
                            right);
                        break;
                    case TokenId.Percent:
                    case TokenId.Identifier:
                        left = Expression.Modulo(left,
                            right);
                        break;
                }
            }
        ...
        private Expression ParseNew()
        {
            ...
            while (true)
            {
                ...
                properties.Add(new DynamicProperty(propName,
                    expr.Type));
                if (token.id != TokenId.Comma)
                    break;
                NextToken();
            ...
            for (int i = 0; i < bindings.Length; i++)
                bindings[i] = Expression.Bind(type.GetProperty(properties[i].Name),
                    expressions[i]);
...
#pragma warning disable 0219
        ...
        /// </summary>
        /// <returns></returns>
        internal IEnumerable<DynamicOrdering> ParseOrdering()
        {
...
#pragma warning restore 0219
        ...
        private Expression ParseRealLiteral()
        {
            ...
            {
                float f;
                if (Single.TryParse(text.Substring(0,
                        text.Length - 1),
                    out f))
                    value = f;
            } else
            ...
            if (value == null)
                throw ParseError(Res.InvalidRealLiteral,
                    text);
            NextToken();
            return CreateLiteral(value,
                text);
        ...
        private Expression ParseStringLiteral()
        {
            ...
            string s = token.text.Substring(1,
                token.text.Length - 2);
            int start = 0;
            while (true)
            {
        ...
        private Expression ParseTypeAccess(Type type)
        {
            ...
            if (token.id == TokenId.OpenParen)
            {
                Expression[] args = ParseArgumentList();
                MethodBase method;
                switch (FindBestMethod(type.GetConstructors(),
                    args,
                    out method))
                {
                    case 0:
                        if (args.Length == 1)
                            return GenerateConversion(args[0],
                                type,
                                errorPos);
                        throw ParseError(errorPos,
                            Res.NoMatchingConstructor,
                            GetTypeName(type));
                    case 1:
            ...
            ValidateToken(TokenId.Dot,
                Res.DotOrOpenParenExpected);
            NextToken();
            return ParseMemberAccess(type,
                null);
        }
        ...
        private Expression ParseUnary()
        {
                ...
                TokenIdentifierIs("not"))
            {
                ...
                {
                    CheckAndPromoteOperand(typeof(INotSignatures),
                        op.text,
                        ref expr,
                        op.pos);
        ...
        private void ProcessValues(object[] values)
        {
            for (int i = 0; i < values.Length; i++)
            {
                ...
                if (i == values.Length - 1 && value is IDictionary<string, object>)
                    ...
                    AddSymbol("@" + i.ToString(CultureInfo.InvariantCulture),
                        value);
            }
        }
