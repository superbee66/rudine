...
using AssociationEndMember = System.Data.Metadata.Edm.AssociationEndMember;
using AssociationType = System.Data.Metadata.Edm.AssociationType;
using BuiltInTypeKind = System.Data.Metadata.Edm.BuiltInTypeKind;
using CollectionType = System.Data.Metadata.Edm.CollectionType;
using ComplexType = System.Data.Metadata.Edm.ComplexType;
using EdmFunction = System.Data.Metadata.Edm.EdmFunction;
using EdmItemCollection = System.Data.Metadata.Edm.EdmItemCollection;
using EdmMember = System.Data.Metadata.Edm.EdmMember;
using EdmProperty = System.Data.Metadata.Edm.EdmProperty;
using EdmSchemaError = System.Data.Metadata.Edm.EdmSchemaError;
using EdmType = System.Data.Metadata.Edm.EdmType;
...
using Facet = System.Data.Metadata.Edm.Facet;
using FunctionParameter = System.Data.Metadata.Edm.FunctionParameter;
using GlobalItem = System.Data.Metadata.Edm.GlobalItem;
using ItemCollection = System.Data.Metadata.Edm.ItemCollection;
using MetadataItem = System.Data.Metadata.Edm.MetadataItem;
using MetadataProperty = System.Data.Metadata.Edm.MetadataProperty;
using MetadataWorkspace = System.Data.Metadata.Edm.MetadataWorkspace;
using NavigationProperty = System.Data.Metadata.Edm.NavigationProperty;
using OperationAction = System.Data.Metadata.Edm.OperationAction;
using ParameterMode = System.Data.Metadata.Edm.ParameterMode;
using PrimitiveType = System.Data.Metadata.Edm.PrimitiveType;
using ReferentialConstraint = System.Data.Metadata.Edm.ReferentialConstraint;
using RefType = System.Data.Metadata.Edm.RefType;
using RelationshipMultiplicity = System.Data.Metadata.Edm.RelationshipMultiplicity;
using SimpleType = System.Data.Metadata.Edm.SimpleType;
using StoreItemCollection = System.Data.Metadata.Edm.StoreItemCollection;
using StructuralType = System.Data.Metadata.Edm.StructuralType;
using TypeUsage = System.Data.Metadata.Edm.TypeUsage;

namespace dCForm.Core.Storage.Sql.Reverser
{
    /// <summary>
    ///     EFCF = Entity Framework Code First
    ///     Class to produce the template output. Base on the official Entity Framework's 6x shipped Entity.tt file, adapted to
    ///     work in .Net 4.0; read https://msdn.microsoft.com/en-us/data/upgradeEF6 as it explains why we didn't just use the
    ///     TextTemplatingFilePreprocessor against the original Entity.tt shipped from MS.
    ///     http://www.olegsych.com/2009/09/t4-preprocessed-text-templates/ also outlines what TextTemplatingFilePreprocessor
    ///     actually is.
    ///     The process of back porting to NET40 is loss on EnumType handling properly.
    ///     TODO:Add back EnumType handling when NET45 is available to this organization at production runtime.
    /// </summary>
    internal class Entity : EntityBase
    {
        ...
        internal virtual string TransformText()
        {
            ...
            TextTemplatingEngineHost efHost = (TextTemplatingEngineHost) Host;
            ...
            Write(ToStringHelper.ToStringWithCulture(code.EscapeNamespace(efHost.Namespace)));


            Write("\r\n{\r\n    public partial class ");


            Write(ToStringHelper.ToStringWithCulture(efHost.EntityType.Name));


            Write("\r\n    {\r\n");


            IEnumerable<NavigationProperty> collectionNavigations = efHost.EntityType.NavigationProperties.Where(
            ...
            if (collectionNavigations.Any())
            {
                ...
                Write(ToStringHelper.ToStringWithCulture(code.Escape(efHost.EntityType)));


                Write("()\r\n        {\r\n");


                foreach (NavigationProperty navProperty in collectionNavigations)
                {
                    Write("            this.");


                    Write(ToStringHelper.ToStringWithCulture(code.Escape(navProperty)));


                    Write(" = new List<");


                    Write(ToStringHelper.ToStringWithCulture(code.Escape(navProperty.ToEndMember.GetEntityType())));
            ...
            foreach (EdmProperty property in efHost.EntityType.Properties)
            {
                string typeUsage = code.Escape(property.TypeUsage);
                ...
                Write(ToStringHelper.ToStringWithCulture(Accessibility.ForProperty(property)));


                Write(" ");


                Write(ToStringHelper.ToStringWithCulture(typeUsage));


                Write(" ");


                Write(ToStringHelper.ToStringWithCulture(code.Escape(property)));


                Write(" { get; set; }\r\n");
            }

            foreach (NavigationProperty navProperty in efHost.EntityType.NavigationProperties.Where(np => np.DeclaringType == efHost.EntityType))
                if (navProperty.ToEndMember.RelationshipMultiplicity == RelationshipMultiplicity.Many)
                {
                    Write("        public virtual ICollection<");
        ...
        public class CodeGenerationTools
        {
            ...
            ///     setting.
            /// </summary>
            public string Escape(Type clrType) { return Escape(clrType, FullyQualifySystemTypes); }
            ...
            private bool VerifyCaseInsensitiveUniqueness(IEnumerable<string> items, Func<string, string> formatMessage, string sourceFile)
            {
                ...
                foreach (string item in items)
                    if (!hash.Add(item))
                    {
                        _textTransformation.Errors.Add(new CompilerError(sourceFile, -1, -1, "6023", formatMessage(item)));
                        return false;
                    }
                return true;
            }
            ...
            /// </summary>
            public IEnumerable<string> GetAllGlobalItems(EdmItemCollection itemCollection) { return itemCollection.GetItems<GlobalItem>().Where(i => i is EntityType || i is ComplexType || i is System.Data.Entity.Core.Metadata.Edm.EdmType || i is EntityContainer).Select(g => GetGlobalItemName(g)); }
            ...
            public static string GetResourceString(string resourceName, CultureInfo culture = null)
            {
                if (_resourceManager == null)
                    _resourceManager = new ResourceManager("System.Data.Entity.Design",
                        typeof (MetadataItemCollectionFactory).Assembly);
            ...
            public IEnumerable<T> GetItemsToGenerate<T>(ItemCollection itemCollection) where T : GlobalItem { return itemCollection.GetItems<T>().Where(i => !i.MetadataProperties.Any(p => p.Name == ExternalTypeNameAttributeName)); }
            ...
            public string GetTypeName(TypeUsage typeUsage) { return typeUsage == null ? null : GetTypeName(typeUsage.EdmType, _ef.IsNullable(typeUsage), null); }
            ...
            /// <returns>The type name to use.</returns>
            public string GetTypeName(EdmType edmType) { return GetTypeName(edmType, null, null); }
            ...
            /// </param>
            /// <returns>The type name to use.</returns>
            public string GetTypeName(TypeUsage typeUsage, string modelNamespace) { return typeUsage == null ? null : GetTypeName(typeUsage.EdmType, _ef.IsNullable(typeUsage), modelNamespace); }
            ...
            /// </param>
            /// <returns>The type name to use.</returns>
            public string GetTypeName(EdmType edmType, string modelNamespace) { return GetTypeName(edmType, null, modelNamespace); }
            ...
            private string GetTypeName(EdmType edmType, bool? isNullable, string modelNamespace)
            {
                ...
                string typeName = Escape(edmType.MetadataProperties
                ...
                {
                    Type clrType = _ef.UnderlyingClrType(edmType);
        ...
        public class MetadataTools
        {
            ...
            /// <summary>
            ///     Given a property on the dependent end of a referential constraint, returns the corresponding property on the
            ///     principal end.
            ///     Requires: The association has a referential constraint, and the specified dependentProperty is one of the
            ///     properties on the dependent end.
            /// </summary>
            public EdmProperty GetCorrespondingPrincipalProperty(NavigationProperty navProperty, EdmProperty dependentProperty)
            {
                ...
                System.Data.Metadata.Edm.ReadOnlyMetadataCollection<EdmProperty> fromProperties = GetPrincipalProperties(navProperty);
                System.Data.Metadata.Edm.ReadOnlyMetadataCollection<EdmProperty> toProperties = GetDependentProperties(navProperty);
                return fromProperties[toProperties.IndexOf(dependentProperty)];
            }

            /// <summary>
            ///     Given a property on the principal end of a referential constraint, returns the corresponding property on the
            ///     dependent end.
            ///     Requires: The association has a referential constraint, and the specified principalProperty is one of the
            ///     properties on the principal end.
            /// </summary>
            ...
            /// <summary>
            ///     Gets the collection of properties that are on the principal end of a referential constraint for the specified
            ///     navigation property.
            ///     Requires: The association has a referential constraint.
            /// </summary>
            public System.Data.Metadata.Edm.ReadOnlyMetadataCollection<EdmProperty> GetPrincipalProperties(NavigationProperty navProperty)
            {
                if (navProperty == null)
                    throw new ArgumentNullException("navProperty");
            ...
            /// <summary>
            ///     Gets the collection of properties that are on the dependent end of a referential constraint for the specified
            ///     navigation property.
            ///     Requires: The association has a referential constraint.
            /// </summary>
            public System.Data.Metadata.Edm.ReadOnlyMetadataCollection<EdmProperty> GetDependentProperties(NavigationProperty navProperty)
            {
                if (navProperty == null)
                    throw new ArgumentNullException("navProperty");
            ...
            ///     In order to be an identifying relationship, the association must have a referential constraint where all of the
            ///     dependent properties are part of the dependent type's primary key.
            /// </summary>
        ...
        /// <summary>
        ///     Responsible for loading an System.Data.Metadata.Edm.EdmItemCollection from a .edmx file or .csdl files
        /// </summary>
        public class MetadataLoader
        {
            ...
            /// <summary>
            ///     Create an System.Data.Metadata.Edm.EdmItemCollection loaded with the metadata provided
            /// </summary>
            ...
            /// <summary>
            ///     Attempts to create a System.Data.Metadata.Edm.EdmItemCollection from the specified metadata file
            /// </summary>
            ...
            /// <summary>
            ///     Attempts to create a StorageMappingItemCollection from the specified metadata file,
            ///     System.Data.Metadata.Edm.EdmItemCollection, and StoreItemCollection
            /// </summary>
            ...
            private abstract class ItemCollectionBuilder
            {
                ...
                /// <summary>
                ///     The name of the XmlElement in the .edmx
                ///     <Runtime>
                ///         element
            ...
            private class StorageMappingItemCollectionBuilder : ItemCollectionBuilder
            {
                ...
                ///     Selects a namespace from the supplied constants.
                /// </summary>
                protected override string GetNamespace(SchemaConstants constants) { return constants.MslNamespace; }
            }
            ...
            private class StoreItemCollectionBuilder : ItemCollectionBuilder
            {
                ...
                ///     Selects a namespace from the supplied constants.
                /// </summary>
                protected override string GetNamespace(SchemaConstants constants) { return constants.SsdlNamespace; }
            }

            /// <summary>
            ///     Builder class for creating a System.Data.Metadata.Edm.EdmItemCollection
            /// </summary>
            private class EdmItemCollectionBuilder : ItemCollectionBuilder
            {
                private readonly List<string> _referenceSchemas = new List<string>();
                ...
                ///     Selects a namespace from the supplied constants.
                /// </summary>
                protected override string GetNamespace(SchemaConstants constants) { return constants.CsdlNamespace; }
            }
        }
        ...
        public static class Accessibility
        {
            ...
            private static string TranslateUserAccessibilityToCSharpAccessibility(string userAccessibility)
            {
                if (userAccessibility == ACCESS_PROTECTED)
                    return "protected";
                if (userAccessibility == ACCESS_INTERNAL)
                    return "internal";
                if (userAccessibility == ACCESS_PRIVATE)
                    return "private";
                // default to public
                return "public";
            }
        ...
        public class EntityFrameworkTemplateFileManager
        {
            ...
            public virtual IEnumerable<string> Process(bool split = true)
            {
                ...
                if (split)
                {
                    ...
                    string headerText = _generationEnvironment.ToString(header.Start, header.Length);
                    string footerText = _generationEnvironment.ToString(footer.Start, footer.Length);
                    string outputPath = Path.GetDirectoryName(_textTransformation.Host.TemplateFile);
        ...
        public struct SchemaConstants
        {
            ...
            public string CsdlNamespace { get; }
            public string SsdlNamespace { get; }
            public string MslNamespace { get; }
            public Version MinimumTemplateVersion { get; }
        }
    }
